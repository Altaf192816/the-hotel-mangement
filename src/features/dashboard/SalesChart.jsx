...
function SalesChart({ bookings, numDays }) {
  ...
  //Todo-->Below code is used to generate dataStructure similar to fakeData above
  
  //?Getting all the dates
  const allDatesArr = eachDayOfInterval({
    //numDays = 7, current date = 7 jan
    start: subDays(new Date(), numDays - 1), //start-->current date - 6  = 1 jan
    end: new Date(), //end-->current date = 7 jan
  });
  //Now we get the array of date from 1 jan to 7 jan
  
  //?creating an array of object same as fakeData above
  const data = allDatesArr.map((date) => {
    return {
      label: format(date, "MMM dd"), //label: "Feb 06"
      totalSales: bookings
        .filter((booking) => isSameDay(new Date(booking.created_at), date)) //filtering booking that have same date-day as date in the allDateArr
        .reduce((acc, cur) => acc + cur.totalPrice, 0), //adding all the booking totalPrice that have same day as present in the date(allDateArr element)
  ...
  return (
    <StyledSalesChart>
      <Heading as="h2">Sales from {format(allDatesArr.at(0),"MMM dd yyyy")} &mdash; {format(allDatesArr.at(-1),"MMM dd yyyy")}</Heading>
